...
async def main():
    async with async_playwright() as p:
        ...
        result_promise = page.evaluate("""
            () => new Promise((resolve) => {
                window.addEventListener('run-sync-bypass-result-check', (e) => {
                    resolve(e.detail);
                }, { once: true });
            })
        """)
        ...
        await page.evaluate("""
            async ({url, payload, token}) => {
                try {
                    ...
                    window.dispatchEvent(new CustomEvent('run-sync-bypass-result-check', { 
        ...
        print("HTML sample (first 1000 chars):")
        print(html[:1000])
