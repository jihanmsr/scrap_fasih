...
async def run():
    async with async_playwright() as p:
        ...
        url = f"https://fasih-sm.bps.go.id/survey-user/api/v1/allocations-view/by-region?surveyPeriodId={survey_period_id}&surveyRoleId={pencacah_id}&page=0&size=5"
        res = await page.evaluate(f"fetch('{url}', {{ headers: {{ 'Accept': 'application/json', 'X-XSRF-TOKEN': '{token}' }} }}).then(r => r.json())")
        print("Response of by-region (new path):", json.dumps(res, indent=2))
