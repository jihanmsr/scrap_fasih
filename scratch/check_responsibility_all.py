...
async def main():
    async with async_playwright() as p:
        ...
        for item in content:
            ...
            if regions:
                print("\nFound officer with regionSummary:")
                print(f"User: {item.get('username')}, Fullname: {item.get('fullName') or item.get('fullname') or '-'}")
        ...
        if not found:
            print("No officer with regionSummary found in first 50 items.")
