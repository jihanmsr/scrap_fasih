...
for fpath in files:
    try:
        ...
        for t in targets:
            status = statuses[t[3]]
            epoch_mod = t[6]
            if status != "OPEN" and epoch_mod > 0:
                d_part = get_wita_date_string(epoch_mod)
                if d_part == "2026-06-22":
                    reg = regions[t[5]]
                    kab_name = reg[1]
                    survey = "se_umum" if t[7] == 0 else "se_ub"
                    key = (kab_name, survey)
                    local_counts[key] = local_counts.get(key, 0) + 1
    except Exception as e:
        print(f"Error processing {fpath}: {e}")

print("\nLocal Counts for 2026-06-22 from partition files:")
total_local = 0
for (kab, survey), count in sorted(local_counts.items(), key=lambda x: x[1], reverse=True):
    print(f"  Kab: {kab}, Survey: {survey}, Count: {count}")
    total_local += count
