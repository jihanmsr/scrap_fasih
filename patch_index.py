...
# 1. Add JS script tag
if 'rekon.js' not in html:
    html = html.replace('<script src="daily_summary.js?v=20260723_v1"></script>', '<script src="daily_summary.js?v=20260723_v1"></script>\n    <script src="rekon.js"></script>')

# 2. Add Tab Button
tab_button = """                <button class="btn-tab" id="tab-btn-rekon" onclick="switchTab('rekon')" title="Rekonsiliasi Data Lintas Sistem">
...
"""
if 'id="tab-btn-rekon"' not in html:
...
tab_content = """
    ...
    <div class="tab-content" id="tab-content-rekon" style="display: none; padding: 1rem;">
        ...
        <div class="filters-container" style="display: flex; gap: 0.5rem; flex-wrap: wrap; margin-bottom: 1rem;">
            <input type="text" id="rekon-filter-search" placeholder="Cari nama/email/SLS..." class="search-input" style="flex: 1; min-width: 200px;" oninput="renderRekon()">
            <select id="rekon-filter-kab" class="filter-select" onchange="renderRekon()"><option value="">Semua Kab</option></select>
            <select id="rekon-filter-kec" class="filter-select" onchange="renderRekon()"><option value="">Semua Kec</option></select>
        </div>
...
"""
if 'id="tab-content-rekon"' not in html:
    html = html.replace('</body>', tab_content + '\n</body>')
