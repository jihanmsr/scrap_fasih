...
async def run():
    async with async_playwright() as p:
        ...
        kab_id = "c523694a-2e72-4570-9489-da2d7b119fe7" # Buol
        ...
        payload = {
            ...
            "assignmentExtraParam": {"region1Id": region1_id, "region2Id": kab_id, "surveyPeriodId": survey_period_id, "assignmentErrorStatusType": -1, "filterTargetType": ""}
        }
        res_dt = await page.evaluate(f"fetch('{url_dt}', {{ method: 'POST', headers: {{ 'Content-Type': 'application/json', 'X-XSRF-TOKEN': '{token}' }}, body: JSON.stringify({json.dumps(payload)}) }}).then(r => r.text())")
        print("Response:", res_dt[:1000])
