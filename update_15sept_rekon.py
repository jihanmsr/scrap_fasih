"""
UPDATE SBR, UTP, KELUARGA - 15 SEPTEMBER 2026
======================================================
1. Menggabungkan 16 CSV SQL Lab dari folder:
   'sbr utp keluarga_15 september'
   -> Rekap SBR, UTP, Keluarga_20260915.xlsx
   -> Rekap SBR, UTP, Keluarga_15_09.xlsx
2. Membuat Rekap Kab/Kot:
   -> Laporan_Rekap_KabKot_SBR_UTP_Keluarga_15_09.xlsx
...
4. Memperbarui Prioritas Penyisiran Keluarga:
   ...
   -> Prioritas_Penyisiran_Keluarga_SE2026_15Sep.xlsx
...
SOURCE_DIR = os.path.join(BASE_DIR, 'sbr utp keluarga_15 september')
...
def step1_combine_csvs():
    print("\n" + "=" * 60)
    print("  [1/5] MENGGABUNGKAN 16 CSV SQL LAB (15 SEPTEMBER 2026)")
    print("=" * 60)

    csv_files = sorted(glob.glob(os.path.join(SOURCE_DIR, '*.csv')))
    print(f"    Ditemukan {len(csv_files)} file CSV di: {SOURCE_DIR}")
    if not csv_files:
        raise FileNotFoundError(f"Tidak ada file CSV di {SOURCE_DIR}")

    dfs = []
    for f in csv_files:
        df_chunk = pd.read_csv(f, dtype={'level_5_full_code': str, 'level_6_code': str})
        dfs.append(df_chunk)

    df_combined = pd.concat(dfs, ignore_index=True)
    initial_len = len(df_combined)
    df_combined = df_combined.drop_duplicates()
    print(f"    Total baris sebelum dedup: {initial_len:,}, setelah dedup: {len(df_combined):,}")
    ...
    df_combined = df_combined.sort_values(by=['level_5_full_code', 'level_6_code'])

    # Save to standard output Excel files
    out_xlsx_1 = os.path.join(BASE_DIR, 'Rekap SBR, UTP, Keluarga_20260915.xlsx')
    out_xlsx_2 = os.path.join(BASE_DIR, 'Rekap SBR, UTP, Keluarga_15_09.xlsx')
...
def step2_rekap_kabkot(df_real):
    print("\n" + "=" * 60)
    print("  [2/5] MEMBUAT LAPORAN REKAP KAB/KOTA (15 SEPTEMBER)")
    print("=" * 60)
    ...
    out_file = os.path.join(BASE_DIR, 'Laporan_Rekap_KabKot_SBR_UTP_Keluarga_15_09.xlsx')
...
def step4_update_penyisiran_keluarga():
    ...
    # Make sure it points to 20260915
    content = re.sub(
...
def step5_update_cache_buster():
    ...
    html = re.sub(r'rekon_data\.js\?v=[^"\'\s>]+', f'rekon_data.js?v={ts}_v1509', html)
    html = re.sub(r'penyisiran_keluarga_data\.js\?v=[^"\'\s>]+', f'penyisiran_keluarga_data.js?v={ts}_v1509', html)
    html = re.sub(r'penyisiran_keluarga\.js\?v=[^"\'\s>]+', f'penyisiran_keluarga.js?v={ts}_v1509', html)

    with open(idx_path, 'w', encoding='utf-8') as f:
        f.write(html)

    print(f"    OK Cache buster index.html diperbarui ke versi v={ts}_v1509")


def main():
    start_time = time.time()
    print("\n" + "#" * 65)
    print("  MEMULAI PEMBARUAN DATA SBR, UTP, KELUARGA (15 SEPTEMBER 2026)")
    print("#" * 65)

    df_real = step1_combine_csvs()
    ...
    print("\n" + "=" * 65)
    print(f"  SEMUA DATA BERHASIL DIUPDATE DALAM {elapsed:.2f} DETIK!")
    print("=" * 65 + "\n")
