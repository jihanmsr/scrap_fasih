...
# ID Provinsi Sulawesi Tengah (SE Umum)
...
async def main():
    async with async_playwright() as p:
        ...
        # Pastikan kita di domain fasih
        await page.goto("https://fasih-sm.bps.go.id/app/dashboard")
        await asyncio.sleep(3) # Tunggu sebentar biar cookie ke-load
        
        cookies = await browser.cookies()
        token_raw = next((c["value"] for c in cookies if c["name"] == "XSRF-TOKEN"), None)
        
        if not token_raw:
            print("Gagal mendapatkan token. Pastikan sudah login.")
            await browser.close()
            return
