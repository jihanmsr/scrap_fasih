    ...
    for idx, row in df.iterrows():
        ...
        try:
            ...
            try:
                ...
                print(f"Failed to decode JSON for {code}: HTTP {r.status_code}. Response: {r.text[:200]}")
                statuses.append('ERROR_DECODE')
                time.sleep(0.1)
                continue
                
            hits = data.get('searchData', [])
            
            # Find exact assignment_id match
            ...
            statuses.append('ERROR_NET')
            
        time.sleep(0.1) # Be nice to the server

    df['status_by_id'] = statuses
    df.to_excel('hasil_bpom_prov_72_kab01_final.xlsx', index=False)
    print(f"Done processing KAB 01. Matches by ID: {len([s for s in statuses if s not in ('NOT_FOUND', 'ERROR_DECODE', 'ERROR_NET')])} / {len(statuses)}")
