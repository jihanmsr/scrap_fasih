...
"""
    if push_code not in content:
        content = content.replace(ending_string, push_code + "\n        " + ending_string)
        with open(filename, "w") as f:
            f.write(content)
        print(f"Modified {filename}")

modify_file("scrape_fast_cdp.py", "await context.close()")
modify_file("scrape_fast_master_sls.py", "await context.close()")
