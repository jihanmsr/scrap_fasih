...
def match_data():
    ...
    df['kode_identitas_16digit'] = codes
    df['status_by_id'] = statuses
    df.to_excel('hasil_bpom_prov_72_full_final.xlsx', index=False)
    print(f"\nDone processing all {len(df)} records. Matches found: {len([s for s in statuses if s not in ('NOT_FOUND', 'ERROR', 'ERROR_DECODE', 'ERROR_NET')])} / {len(statuses)}")
