"""
UPDATE 31 AGUSTUS 2026 - Master Script
================================================
Mengupdate seluruh menu dashboard secara komprehensif dari folder update_31/:
1. [PROGRES PETUGAS]  rekap petugas 31.xlsx
                      -> fast_petugas_progress.js + petugas_region_map.js
                      -> fast_petugas_all_2026-08-31.csv -> fast_petugas_history.js
                      -> Rekap Progress Petugas 31_08.xlsx
2. [REKON SBR/UTP]    rekap_sbr_utp_keluarga (4).xlsx
                      -> Rekap SBR, UTP, Keluarga_20260831.xlsx
                      -> Laporan_Rekap_KabKot_SBR_UTP_Keluarga_31_08.xlsx
                      -> rekon_data.js
3. [SLS OPEN]         Progress Petugas + jumlah_subsls_yang_belum_dikunjungi (4).xlsx
...
UPDATE_DIR = os.path.join(BASE_DIR, 'update_31')

# File paths in update_31
F_PROGRES = os.path.join(UPDATE_DIR, 'rekap petugas 31.xlsx')
...
def update_progres_petugas():
    print("\n" + "="*60)
    print("  [1/3] UPDATE MENU PROGRES PETUGAS & HISTORY (31 AGUSTUS)")
    print("="*60)
    ...
    # Save to Excel
    out_xlsx = os.path.join(BASE_DIR, 'Rekap Progress Petugas 31_08.xlsx')
    try:
    ...
    # ── B. fast_petugas_all_2026-08-31.csv & fast_petugas_history.js ────────────
    ...
    csv_31 = os.path.join(BASE_DIR, 'fast_petugas_all_2026-08-31.csv')
    combined.to_csv(csv_31, index=False)
    combined.to_csv(os.path.join(BASE_DIR, 'fast_petugas_all.csv'), index=False)
    print(f"    OK fast_petugas_all_2026-08-31.csv dibuat ({len(combined):,} baris)")

    # Rebuild history
    import subprocess
    subprocess.run([sys.executable, os.path.join(BASE_DIR, 'rebuild_history.py')], check=True)
    print("    OK fast_petugas_history.js diperbarui (sampai 31 AGUSTUS)")
...
def update_rekon_sbr():
    print("\n" + "="*60)
    print("  [2/3] UPDATE MENU REKON / SBR-UTP-KELUARGA (TABULASI 31 AGUSTUS)")
    print("="*60)
    ...
    # Save to Excel
    out_rekon_xlsx = os.path.join(BASE_DIR, 'Rekap SBR, UTP, Keluarga_20260831.xlsx')
    df_real.to_excel(out_rekon_xlsx, index=False)
    out_rekon_xlsx_alt = os.path.join(BASE_DIR, 'Rekap SBR, UTP, Keluarga_31_08.xlsx')
    ...
    out_kabkot = os.path.join(BASE_DIR, 'Laporan_Rekap_KabKot_SBR_UTP_Keluarga_31_08.xlsx')
...
def update_cache_buster():
    ...
    if os.path.exists(idx_path):
        ...
        html = re.sub(r'v=\d+(_v\d+)?', f'v={ts}_v31', html)
        with open(idx_path, 'w', encoding='utf-8') as f:
            f.write(html)
        print(f"\n    OK Cache buster index.html diperbarui ke: v={ts}_v31")
...
def main():
    print("\n" + "=" * 60)
    print("  UPDATE DASHBOARD 31 AGUSTUS 2026")
    print("=" * 60)
    ...
    print("\n" + "=" * 60)
    print("  SEMUA DATA BERHASIL DIUPDATE PER 31 AGUSTUS 2026!")
    print("=" * 60 + "\n")
