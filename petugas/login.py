...
def login_with_sso(username, password, otp_code=None):
    ...
    try:
        print('Cek Button Lanjutkan dengan SSO')
        sso_btn = page.locator("text='Lanjutkan dengan SSO'")
        if sso_btn.count() > 0:
            sso_btn.first.click()
            print("berhasil klik button lanjutkan dengan SSO, menunggu redirect...")
            page.wait_for_url("https://fasih-sm.bps.go.id/survey-collection/survey", timeout=60000)
            current_url = page.url
            print('Check fasih url sesudah klik button : ', current_url)
            
    except Exception:
        pass
    
    try:
        print('Cek Button Refresh Page')
        refresh_btn = page.locator("button:has-text('Refresh Page')")
        if refresh_btn.count() > 0:
            refresh_btn.first.click()
            print("Berhasil klik button Refresh Page, menunggu redirect...")
            page.wait_for_load_state('networkidle', timeout=60000)
            current_url = page.url
            print('Check url sesudah klik Refresh Page : ', current_url)
    except Exception:
        pass
