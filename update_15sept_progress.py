"""
UPDATE PROGRESS PETUGAS - 15 SEPTEMBER 2026
======================================================
1. Menggabungkan data progres Kota Palu (15 September 2026) dari:
   'sqllab_rekap_progress_petugas_20260915T131704.csv'
   dengan data kabupaten lain dari master:
   'update_8/rekap_progress_petugas (7).xlsx'
2. Menyimpan Rekap Progress Petugas 15_09.xlsx & rekap_progress_petugas (1).xlsx
3. Memperbarui fast_petugas_progress.js & petugas_region_map.js
4. Membuat fast_petugas_all_2026-09-15.csv & update fast_petugas_all.csv
5. Menjalankan rebuild_history.py -> fast_petugas_history.js
6. Memperbarui rekap_status_sls.js
...
PALU_CSV = os.path.join(BASE_DIR, 'sqllab_rekap_progress_petugas_20260915T131704.csv')
MASTER_XLSX = os.path.join(BASE_DIR, 'update_8', 'rekap_progress_petugas (7).xlsx')
if not os.path.exists(MASTER_XLSX):
    MASTER_XLSX = os.path.join(BASE_DIR, 'Rekap Progress Petugas 01_09.xlsx')
...
def step1_combine_progress():
    print("\n" + "=" * 60)
    print("  [1/6] MEMUAT & MENGGABUNGKAN DATA PROGRES PETUGAS")
    print("=" * 60)

    print(f"    Membaca master progres lama: {os.path.basename(MASTER_XLSX)}...")
    df_master = pd.read_excel(MASTER_XLSX, dtype=str)
    print(f"    -> {len(df_master):,} baris master lama terbaca")

    print(f"    Membaca update progres Palu: {os.path.basename(PALU_CSV)}...")
    df_palu = pd.read_csv(PALU_CSV, dtype=str)
    print(f"    -> {len(df_palu):,} baris update Palu terbaca")

    # Filter out old Palu (7271)
    df_other = df_master[df_master['level_5_full_code'].astype(str).str.replace('.0', '', regex=False).str.strip().str[:4] != '7271'].copy()
    print(f"    -> {len(df_other):,} baris kabupaten lain dipertahankan")

    df_combined = pd.concat([df_other, df_palu], ignore_index=True)
    print(f"    -> Total setelah digabung: {len(df_combined):,} baris")

    # Clean numeric columns
    for c in numeric_cols:
        if c in df_combined.columns:
            df_combined[c] = pd.to_numeric(df_combined[c], errors='coerce').fillna(0).astype(int)

    # Standardize codes
    df_combined['level_5_full_code'] = df_combined['level_5_full_code'].astype(str).str.replace(r'\.0$', '', regex=True).str.strip()
    df_combined['level_6_code'] = pd.to_numeric(df_combined['level_6_code'], errors='coerce').fillna(0).astype(int).astype(str).str.zfill(2)

    # Save to Excel
    out_xlsx_1 = os.path.join(BASE_DIR, 'Rekap Progress Petugas 15_09.xlsx')
    out_xlsx_2 = os.path.join(BASE_DIR, 'rekap_progress_petugas (1).xlsx')
    print(f"    Menyimpan ke {os.path.basename(out_xlsx_1)}...")
    df_combined.to_excel(out_xlsx_1, index=False)
    print(f"    Menyimpan ke {os.path.basename(out_xlsx_2)}...")
    df_combined.to_excel(out_xlsx_2, index=False)

    print("    OK Penggabungan data progres berhasil!")
    return df_combined


def step2_update_fast_petugas_progress(df):
    print("\n" + "=" * 60)
    print("  [2/6] MEMPERBARUI fast_petugas_progress.js & petugas_region_map.js")
    print("=" * 60)
    ...
    reg_map = json.loads(match_r.group(1)) if match_r else {}
    ...
    content_reg = content_reg[:match_r.start(1)] + new_reg_json + content_reg[match_r.end(1):]
...
def step3_create_fast_petugas_all(df):
    print("\n" + "=" * 60)
    print("  [3/6] MEMBUAT fast_petugas_all_2026-09-15.csv & fast_petugas_all.csv")
    print("=" * 60)
    ...
    csv_15 = os.path.join(BASE_DIR, 'fast_petugas_all_2026-09-15.csv')
    combined.to_csv(csv_15, index=False)
    combined.to_csv(os.path.join(BASE_DIR, 'fast_petugas_all.csv'), index=False)
    print(f"    OK fast_petugas_all_2026-09-15.csv dibuat ({len(combined):,} petugas)")


def step4_rebuild_history():
    print("\n" + "=" * 60)
    print("  [4/6] MENJALANKAN rebuild_history.py")
    print("=" * 60)
    subprocess.run([sys.executable, os.path.join(BASE_DIR, 'rebuild_history.py')], check=True)
    print("    OK fast_petugas_history.js diperbarui (sampai 15 SEPTEMBER 2026)")


def step5_update_rekap_status_sls(df_prog):
    print("\n" + "=" * 60)
    print("  [5/6] MEMPERBARUI rekap_status_sls.js")
    print("=" * 60)
...
def step6_update_cache_buster():
    print("\n" + "=" * 60)
    print("  [6/6] MEMPERBARUI CACHE BUSTER INDEX.HTML")
    print("=" * 60)
    ...
    html = re.sub(r'fast_petugas_progress\.js\?v=[^"\'\s>]+', f'fast_petugas_progress.js?v={ts}_v1509', html)
    html = re.sub(r'fast_petugas_history\.js\?v=[^"\'\s>]+', f'fast_petugas_history.js?v={ts}_v1509', html)
    html = re.sub(r'rekap_status_sls\.js\?v=[^"\'\s>]+', f'rekap_status_sls.js?v={ts}_v1509', html)

    with open(idx_path, 'w', encoding='utf-8') as f:
        f.write(html)

    print(f"    OK Cache buster index.html diperbarui!")


def main():
    start_time = time.time()
    print("\n" + "#" * 65)
    print("  MEMULAI PEMBARUAN PROGRES PETUGAS (15 SEPTEMBER 2026)")
    print("#" * 65)

    df = step1_combine_progress()
    ...
    step6_update_cache_buster()

    elapsed = time.time() - start_time
    print("\n" + "=" * 65)
    print(f"  SEMUA DATA PROGRES BERHASIL DIUPDATE DALAM {elapsed:.2f} DETIK!")
    print("=" * 65 + "\n")
