"""
UPDATE PROGRESS PETUGAS - 18 SEPTEMBER 2026
======================================================
1. Membaca file 'rekap_progress_petugas_18 september.xlsx'
   -> Rekap Progress Petugas 18_09.xlsx
   -> rekap_progress_petugas (1).xlsx
2. Memperbarui fast_petugas_progress.js & petugas_region_map.js
3. Membuat fast_petugas_all_2026-09-18.csv & update fast_petugas_all.csv
4. Menjalankan rebuild_history.py -> fast_petugas_history.js
5. Memperbarui rekap_status_sls.js (Window.REKAP_STATUS_DATA)
6. Menjalankan generate_keberadaan_data.py & build_monitoring_lanjutan_data.py
7. Memperbarui cache buster di index.html
"""
...
SOURCE_XLSX = os.path.join(BASE_DIR, 'rekap_progress_petugas_18 september.xlsx')
...
def step1_load_progress():
    print("\n" + "=" * 60)
    print("  [1/7] MEMBACA & MENYIMPAN EXCEL PROGRES 18 SEPTEMBER 2026")
    print("=" * 60)

    if not os.path.exists(SOURCE_XLSX):
        raise FileNotFoundError(f"File tidak ditemukan: {SOURCE_XLSX}")

    print(f"    Membaca: {os.path.basename(SOURCE_XLSX)}...")
    df = pd.read_excel(SOURCE_XLSX, dtype={'level_5_full_code': str, 'level_6_code': str})
    print(f"    -> {len(df):,} baris progres terbaca")
    ...
    # Save to Excel
    out_xlsx_1 = os.path.join(BASE_DIR, 'Rekap Progress Petugas 18_09.xlsx')
    out_xlsx_2 = os.path.join(BASE_DIR, 'rekap_progress_petugas (1).xlsx')
    ...
    print("    OK Simpan Excel progres berhasil!")
    return df


def step2_update_fast_petugas_progress(df):
    print("\n" + "=" * 60)
    print("  [2/7] MEMPERBARUI fast_petugas_progress.js & petugas_region_map.js")
    print("=" * 60)
    ...
    prefix = 'window.PETUGAS_PROGRESS_MAP = '
    idx = content_prog.find(prefix)
    if idx == -1:
        raise ValueError("Could not find window.PETUGAS_PROGRESS_MAP in fast_petugas_progress.js")

    json_str = content_prog[idx + len(prefix):].rstrip(';\n ')
    prog_map = json.loads(json_str)
    if 'Pencacah' not in prog_map:
        prog_map['Pencacah'] = {}
    if 'Pengawas' not in prog_map:
        prog_map['Pengawas'] = {}
    ...
    prefix_r = 'window.PETUGAS_REGION_MAP = '
    idx_r = content_reg.find(prefix_r)
    reg_map = json.loads(content_reg[idx_r + len(prefix_r):].rstrip(';\n ')) if idx_r != -1 else {}
    ...
    for _, row in df_p.iterrows():
        ...
        reg_code = str(row['level_5_full_code']).strip()
        subsls_code = str(row['level_6_code']).strip().zfill(2)
        sls_id_16 = reg_code + subsls_code

        if email not in progress_pencacah:
            progress_pencacah[email] = {
        ...
        p = progress_pencacah[email]
        p['target'] += target_row
        p['open'] += g('open')
        p['draft'] += g('draft')
        p['submitted_respondent'] += g('submitted_respondent')
        p['submitted_pencacah'] += g('submitted_by_pencacah')
        p['approved'] += g('approved_by_pengawas')
        p['rejected'] += g('rejected_by_pengawas') + g('rejected_by_admin_kabupaten')
        p['revoked'] += g('revoked_by_pengawas')
        p['edited_pengawas'] += g('edited_by_pengawas')
        p['edited_admin'] += g('edited_by_admin_kabupaten')
        p['completed_admin'] += g('completed_by_admin_kabupaten')

        # SLS Details
        status_dict = {}
        if g('open') > 0: status_dict['OPEN'] = g('open')
        if g('draft') > 0: status_dict['DRAFT'] = g('draft')
        if g('submitted_respondent') > 0: status_dict['SUBMITTED RESPONDENT'] = g('submitted_respondent')
        if g('submitted_by_pencacah') > 0: status_dict['SUBMITTED BY PENCACAH'] = g('submitted_by_pencacah')
        if g('approved_by_pengawas') > 0: status_dict['APPROVED BY PENGAWAS'] = g('approved_by_pengawas')
        if g('rejected_by_pengawas') > 0: status_dict['REJECTED BY PENGAWAS'] = g('rejected_by_pengawas')
        if g('revoked_by_pengawas') > 0: status_dict['REVOKED BY PENGAWAS'] = g('revoked_by_pengawas')
        if g('edited_by_pengawas') > 0: status_dict['EDITED BY PENGAWAS'] = g('edited_by_pengawas')
        if g('edited_by_admin_kabupaten') > 0: status_dict['EDITED BY ADMIN KABUPATEN'] = g('edited_by_admin_kabupaten')
        if g('rejected_by_admin_kabupaten') > 0: status_dict['REJECTED BY ADMIN KABUPATEN'] = g('rejected_by_admin_kabupaten')
        if g('completed_by_admin_kabupaten') > 0: status_dict['COMPLETED BY ADMIN KABUPATEN'] = g('completed_by_admin_kabupaten')

        p['sls_details'][sls_id_16] = {
            'total': target_row,
            'status': status_dict
        }
    ...
    for _, row in df_w.iterrows():
        ...
        if email not in progress_pengawas:
            progress_pengawas[email] = {
                'target': 0, 'submitted_pencacah': 0, 'submitted_respondent': 0,
                'approved': 0, 'rejected': 0, 'draft': 0, 'open': 0, 'revoked': 0,
                'edited_pengawas': 0, 'edited_admin': 0, 'completed_admin': 0,
                'sls_details': {}
        ...
        w = progress_pengawas[email]
        w['target'] += target_row
        w['open'] += g('open')
        w['draft'] += g('draft')
        w['submitted_respondent'] += g('submitted_respondent')
        w['submitted_pencacah'] += g('submitted_by_pencacah')
        w['approved'] += g('approved_by_pengawas')
        w['rejected'] += g('rejected_by_pengawas') + g('rejected_by_admin_kabupaten')
        w['revoked'] += g('revoked_by_pengawas')
        w['edited_pengawas'] += g('edited_by_pengawas')
        w['edited_admin'] += g('edited_by_admin_kabupaten')
        w['completed_admin'] += g('completed_by_admin_kabupaten')
        ...
        w['sls_details'][sls_id_16] = {
            'total': target_row,
            'status': status_dict
        }

    # Merge into prog_map
    for email, data in progress_pencacah.items():
        prog_map['Pencacah'][email] = data

    for email, data in progress_pengawas.items():
        prog_map['Pengawas'][email] = data

    with open(prog_path, 'w', encoding='utf-8') as f:
        f.write('window.PETUGAS_PROGRESS_MAP = ' + json.dumps(prog_map, indent=4, ensure_ascii=False) + ';\n')

    print(f"    OK fast_petugas_progress.js diperbarui ({len(prog_map['Pencacah'])} Pencacah, {len(prog_map['Pengawas'])} Pengawas)")
    ...
    with open(reg_path, 'w', encoding='utf-8') as f:
        f.write('window.PETUGAS_REGION_MAP = ' + json.dumps(reg_map, indent=4, ensure_ascii=False) + ';\n')

    print(f"    OK petugas_region_map.js diperbarui (+{added_r} email baru)")


def step3_create_fast_petugas_all(df):
    print("\n" + "=" * 60)
    print("  [3/7] MEMBUAT fast_petugas_all_2026-09-18.csv & fast_petugas_all.csv")
    print("=" * 60)
    ...
    csv_18 = os.path.join(BASE_DIR, 'fast_petugas_all_2026-09-18.csv')
    combined.to_csv(csv_18, index=False)
    combined.to_csv(os.path.join(BASE_DIR, 'fast_petugas_all.csv'), index=False)
    print(f"    OK fast_petugas_all_2026-09-18.csv dibuat ({len(combined):,} petugas)")


def step4_rebuild_history():
    print("\n" + "=" * 60)
    print("  [4/7] MENJALANKAN rebuild_history.py")
    print("=" * 60)
    subprocess.run([sys.executable, os.path.join(BASE_DIR, 'rebuild_history.py')], check=True)
    print("    OK fast_petugas_history.js diperbarui (sampai 18 SEPTEMBER 2026)")


def step5_update_rekap_status_sls(df_prog):
    print("\n" + "=" * 60)
    print("  [5/7] MEMPERBARUI rekap_status_sls.js")
    print("=" * 60)
...
def step6_run_satellite_and_keberadaan():
    print("\n" + "=" * 60)
    print("  [6/7] MEMPERBARUI DATA KEBERADAAN PETUGAS & MONITORING LANJUTAN")
    print("=" * 60)

    # 1. generate_keberadaan_data.py
    py_keb = os.path.join(BASE_DIR, 'generate_keberadaan_data.py')
    if os.path.exists(py_keb):
        print(f"    Menjalankan {os.path.basename(py_keb)}...")
        subprocess.run([sys.executable, py_keb], check=True)
        print("    OK data_keberadaan_petugas.js diperbarui!")

    # 2. build_monitoring_lanjutan_data.py
    py_mon = os.path.join(BASE_DIR, 'build_monitoring_lanjutan_data.py')
    if os.path.exists(py_mon):
        print(f"    Menjalankan {os.path.basename(py_mon)}...")
        subprocess.run([sys.executable, py_mon], check=True)
        print("    OK monitoring_lanjutan_data.js & palu_monitoring_data.js diperbarui!")


def step7_update_cache_buster():
    print("\n" + "=" * 60)
    print("  [7/7] MEMPERBARUI CACHE BUSTER INDEX.HTML")
    print("=" * 60)
    ...
    html = re.sub(r'fast_petugas_progress\.js\?v=[^"\'\s>]+', f'fast_petugas_progress.js?v={ts}_v1809', html)
    html = re.sub(r'fast_petugas_history\.js\?v=[^"\'\s>]+', f'fast_petugas_history.js?v={ts}_v1809', html)
    html = re.sub(r'petugas_region_map\.js\?v=[^"\'\s>]+', f'petugas_region_map.js?v={ts}_v1809', html)
    html = re.sub(r'rekap_status_sls\.js\?v=[^"\'\s>]+', f'rekap_status_sls.js?v={ts}_v1809', html)
    html = re.sub(r'data_keberadaan_petugas\.js\?v=[^"\'\s>]+', f'data_keberadaan_petugas.js?v={ts}_v1809', html)
    html = re.sub(r'palu_monitoring_data\.js\?v=[^"\'\s>]+', f'palu_monitoring_data.js?v={ts}_v1809', html)
...
def main():
    ...
    print("\n" + "#" * 65)
    print("  MEMULAI PEMBARUAN PROGRES PETUGAS (18 SEPTEMBER 2026)")
    print("#" * 65)

    df = step1_load_progress()
    step2_update_fast_petugas_progress(df)
    step3_create_fast_petugas_all(df)
    step4_rebuild_history()
    step5_update_rekap_status_sls(df)
    step6_run_satellite_and_keberadaan()
    step7_update_cache_buster()

    elapsed = time.time() - start_time
    print("\n" + "=" * 65)
    print(f"  SEMUA DATA PROGRES PETUGAS BERHASIL DIUPDATE DALAM {elapsed:.2f} DETIK!")
    print("=" * 65 + "\n")
