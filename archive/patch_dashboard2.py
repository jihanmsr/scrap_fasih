...
# Modify renderDashboardStats properly
new_render_dashboard = """
async function renderDashboardStats() {
    const surveyType = activeSubtab === 'se2026' ? 'se_umum' : 'se_ub';
    const prefix = activeSubtab === 'se2026' ? 'se_umum' : 'se_ub';
    const kabFilterEl = document.getElementById(activeSubtab === 'se2026' ? 'filter-kabupaten' : 'ub-filter-kabupaten');
    const kabVal = kabFilterEl ? kabFilterEl.value : 'all';
    
    // FETCH DASHBOARD SUMMARY DARI MYSQL!
    const cleanKabVal = kabVal.replace(/^\[\\d+\]\\s*/, '').trim().toUpperCase();
    
    document.getElementById(`${prefix}-stat-total-prelist`).innerHTML = '<span style="font-size: 0.9rem; color: var(--text-secondary);">Memuat...</span>';
    document.getElementById(`${prefix}-stat-submitted`).innerHTML = '<span style="font-size: 0.9rem; color: var(--text-secondary);">Memuat...</span>';
    ...
    try {
        ...
        data.forEach((row, idx) => {
            ...
            <tr style="border-bottom: 1px solid var(--card-border); transition: background 0.2s ease;">
                ...
                <td style="padding: 0.8rem 1rem; font-weight: 700; color: var(--text-primary); cursor: pointer;" onclick="if('${kabVal}' === 'all') { const sel = document.getElementById(activeSubtab === 'se2026' ? 'filter-kabupaten' : 'ub-filter-kabupaten'); if(sel) { for(let i=0; i<sel.options.length; i++) { if(sel.options[i].text.includes('${name}')) { sel.selectedIndex = i; sel.dispatchEvent(new Event('change')); break; } } } }">${name}</td>
        ...
        // Update KPI Cards
        document.getElementById(`${prefix}-stat-total-prelist`).innerHTML = `${totalTarget.toLocaleString('id-ID')}`;
        
        const pctTotalSelesai = totalTarget > 0 ? ((totalSelesai / totalTarget) * 100).toFixed(2) : '0.00';
        document.getElementById(`${prefix}-stat-submitted`).innerHTML = `${totalSelesai.toLocaleString('id-ID')}`;
        const pctEl = document.getElementById(`${prefix}-stat-percentage`);
        if (pctEl) {
            pctEl.innerText = `(${pctTotalSelesai}%)`;
            pctEl.style.color = pctTotalSelesai >= 50 ? 'var(--color-delivered)' : (pctTotalSelesai > 0 ? '#f59e0b' : 'var(--text-secondary)');
        }
        
        // Update Chart
        const chartObj = activeSubtab === 'se2026' ? window.capaianChart : window.ubCapaianChart;
        if (chartObj) {
            chartObj.data.labels = chartLabels;
            chartObj.data.datasets[0].data = chartSelesai;
            chartObj.data.datasets[1].data = chartTarget;
            chartObj.update();
        }
        
        // Update Table
        const tbody = document.getElementById(activeSubtab === 'se2026' ? 'se_umum-rincian-tbody' : 'se_ub-rincian-tbody');
        if (tbody) {
            if (data.length === 0) {
