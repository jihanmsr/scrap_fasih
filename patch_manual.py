...
# =========================================================
# GANTI BAGIAN INI SESUAI DENGAN YANG ERROR DI TERMINAL:
ROLE = "Pencacah"       # Pilih "Pencacah" atau "Pengawas"
KABUPATEN = "Banggai"   # Ganti dengan nama kabupaten yang error tadi
HALAMAN = 5             # Halaman yang terlewat
# =========================================================
...
async def run():
    target_kabupaten_name = KABUPATEN
    print(f"[INFO] Menambal halaman manual untuk Role: {ROLE} - {target_kabupaten_name} Halaman {HALRAPAN if 'HALAMAN' not in globals() else HALAMAN}")
    ...
    for kab_code, kab_info in kabupaten_list.items():
        if target_kabupaten_name.upper() in kab_info["kab_name"].upper():
            kab_id = kab_info["kab_id"]
            target_kabupaten_name = kab_info["kab_name"] # Ambil nama full dari json
            break
    
    if not kab_id:
        print(f"[ERROR] Kabupaten '{KABUPATEN}' tidak ditemukan di sistem. Pastikan ejaannya benar.")
        return
        
    role_id = ROLES.get(ROLE)
    if not role_id:
        print(f"[ERROR] Role '{ROLE}' tidak valid. Harus 'Pencacah' atau 'Pengawas'.")
        return

    print(f"[INFO] Target ditemukan: {target_kabupaten_name} (ID: {kab_id})")
    ...
    async with async_playwright() as p:
        ...
        while True:
            ...
            if res and "_error" in res:
                retries += 1
                print(f"[ERROR] Gagal mengambil halaman {HALAMAN} (Percobaan {retries}/{max_retries}): {res.get('_error')}")
                ...
                if retries >= max_retries:
                    print(f"[WARNING] Gagal total setelah {max_retries} percobaan di halaman {HALAMAN}. PATCH GAGAL!")
                    break
            ...
            if not content:
                print(f"    [INFO] Halaman {HALAMAN} kosong.")
                break
            
            print(f"    [INFO] Sukses menarik {len(content)} data pada halaman {HALAMAN}.")
            for c in content:
        ...
        print(f"✅ Penambalan manual selesai. Data bolong berhasil disisipkan ke {csv_file}")
        ...
        try:
            ...
            subprocess.run(["git", "commit", "-m", "Patch manual missing data"], check=True)
