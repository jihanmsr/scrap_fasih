"""
UPDATE 30 AGUSTUS 2026 - Master Script
================================================
Mengupdate seluruh menu dashboard secara komprehensif dari folder update_30/:
1. [PROGRES PETUGAS]  2 file sqllab_rekap_progress_petugas_20260830T*.csv
                      -> fast_petugas_progress.js + petugas_region_map.js
                      -> fast_petugas_all_2026-08-30.csv -> fast_petugas_history.js
                      -> Rekap Progress Petugas 30_08.xlsx
2. [REKON SBR/UTP]    2 file sqllab_rekap_sbr_utp_keluarga_20260830T*.csv
                      -> Rekap SBR, UTP, Keluarga_20260830.xlsx
                      -> Laporan_Rekap_KabKot_SBR_UTP_Keluarga_30_08.xlsx
                      -> rekon_data.js
3. [SLS OPEN]         Progress Petugas + sqllab_rekap_jumlah_sub_sls_belum_dikunjungi_20260830T133853.csv
...
UPDATE_DIR = os.path.join(BASE_DIR, 'update_30')

# File paths in update_30
F_PROG_1 = os.path.join(UPDATE_DIR, 'sqllab_rekap_progress_petugas_20260830T133914.csv')
F_PROG_2 = os.path.join(UPDATE_DIR, 'sqllab_rekap_progress_petugas_20260830T133932.csv')

F_REKON_1 = os.path.join(UPDATE_DIR, 'sqllab_rekap_sbr_utp_keluarga_20260830T133955.csv')
F_REKON_2 = os.path.join(UPDATE_DIR, 'sqllab_rekap_sbr_utp_keluarga_20260830T134001.csv')

F_SUBSLS  = os.path.join(UPDATE_DIR, 'sqllab_rekap_jumlah_sub_sls_belum_dikunjungi_20260830T133853.csv')
...
def update_progres_petugas():
    print("\n" + "="*60)
    print("  [1/3] UPDATE MENU PROGRES PETUGAS & HISTORY (30 AGUSTUS)")
    print("="*60)

    df1 = pd.read_csv(F_PROG_1, dtype=str)
    df2 = pd.read_csv(F_PROG_2, dtype=str)
    df = pd.concat([df1, df2], ignore_index=True)
    print(f"    -> {len(df):,} baris data progres petugas digabung")
    ...
    # Save to Excel
    out_xlsx = os.path.join(BASE_DIR, 'Rekap Progress Petugas 30_08.xlsx')
    try:
    ...
    # ── B. fast_petugas_all_2026-08-30.csv & fast_petugas_history.js ────────────
    ...
    csv_30 = os.path.join(BASE_DIR, 'fast_petugas_all_2026-08-30.csv')
    combined.to_csv(csv_30, index=False)
    combined.to_csv(os.path.join(BASE_DIR, 'fast_petugas_all.csv'), index=False)
    print(f"    OK fast_petugas_all_2026-08-30.csv dibuat ({len(combined):,} baris)")

    # Rebuild history
    import subprocess
    subprocess.run([sys.executable, os.path.join(BASE_DIR, 'rebuild_history.py')], check=True)
    print("    OK fast_petugas_history.js diperbarui (sampai 30 AGUSTUS)")
...
def update_rekon_sbr():
    print("\n" + "="*60)
    print("  [2/3] UPDATE MENU REKON / SBR-UTP-KELUARGA (TABULASI 30 AGUSTUS)")
    print("="*60)

    df_r1 = pd.read_csv(F_REKON_1, dtype={'level_5_full_code': str, 'level_6_code': str})
    df_r2 = pd.read_csv(F_REKON_2, dtype={'level_5_full_code': str, 'level_6_code': str})
    df_real = pd.concat([df_r1, df_r2], ignore_index=True)
    print(f"    -> {len(df_real):,} baris realisasi SBR/UTP digabung")

    # Save to Excel
    out_rekon_xlsx = os.path.join(BASE_DIR, 'Rekap SBR, UTP, Keluarga_20260830.xlsx')
    df_real.to_excel(out_rekon_xlsx, index=False)
    out_rekon_xlsx_alt = os.path.join(BASE_DIR, 'Rekap SBR, UTP, Keluarga_30_08.xlsx')
    ...
    out_kabkot = os.path.join(BASE_DIR, 'Laporan_Rekap_KabKot_SBR_UTP_Keluarga_30_08.xlsx')
...
def update_sls_open(df_prog=None):
    ...
    if df_prog is None:
        df1 = pd.read_csv(F_PROG_1, dtype=str)
        df2 = pd.read_csv(F_PROG_2, dtype=str)
        df_prog = pd.concat([df1, df2], ignore_index=True)
...
def update_cache_buster():
    ...
    if os.path.exists(idx_path):
        ...
        html = re.sub(r'v=\d+(_v\d+)?', f'v={ts}_v30', html)
        with open(idx_path, 'w', encoding='utf-8') as f:
            f.write(html)
        print(f"\n    OK Cache buster index.html diperbarui ke: v={ts}_v30")
...
def main():
    print("\n" + "=" * 60)
    print("  UPDATE DASHBOARD 30 AGUSTUS 2026")
    print("=" * 60)
    ...
    print("\n" + "=" * 60)
    print("  SEMUA DATA BERHASIL DIUPDATE PER 30 AGUSTUS 2026!")
    print("=" * 60 + "\n")
