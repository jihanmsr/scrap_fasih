...
def main():
    ...
    kode_kab = '7204'
    # Look up kab_id from kab.json using kode_kab
    kab_id = None
    try:
        with open("kab.json", "r", encoding="utf-8") as f:
            kab_data = json.load(f)
        for item in kab_data.get("data", []):
            if item.get("fullCode") == kode_kab:
                kab_id = item.get("id")
                break
    except Exception as e:
        print(f"Gagal memuat kab_id dari kab.json: {e}")
    ...
    try:
        ...
        # Filter kecamatan list from kec.json by kode_kab
        headers_new = {
            "accept": "*/*",
        ...
        def fetch_role_data(role_id, role_name):
        ...
        try:
            fetch_role_data(pengawas_id, "pengawas")
            fetch_role_data(pencacah_id, "pencacah")
